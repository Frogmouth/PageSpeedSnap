...
const DOMsniffer = function() {
	...
	PANELS.forEach((pan) => {
		...
		//grab vitals
		result[panName] = {
			general: pan[ONE](SELECTORS.VITALS_GENERAL).nextElementSibling[ONE]('span')?.innerText || null,
		...
		Object.keys(vitalsTrashold).forEach(function (metric) {
			...
			result[panName].metrics[metric] = {
				...
				display : cleanedText,
				value : value || null,
				assessment : !value ? null : value <= vitalsTrashold[metric][0] ? 'good' : value <= vitalsTrashold[metric][1] ? 'average' : 'bad',
                percentages : $vitalValue?.children?.[$vitalValue.children.length-1]?.innerText.match(/\d+%/g)?.map((p) => ({displayValue: p, value: parseInt(p)})) || [],
			};
		});
	})
...
const sniffer = function () {
    ...
    if(window.__LIGHTHOUSE_MOBILE_JSON__) {
        currentMobileConsistencyVersion = getSniffVersion(window.__LIGHTHOUSE_MOBILE_JSON__);
        mobileNewVersion = currentMobileConsistencyVersion !== window.mobileConsistencyVersion;
    } else {
        console.log("PAGESPEED SNAP - SNIFFER - MOBILE missing metrics");
        return false;
    }
    if(window.__LIGHTHOUSE_DESKTOP_JSON__) {
        currentDesktopConsistencyVersion = getSniffVersion(window.__LIGHTHOUSE_DESKTOP_JSON__);
        desktopNewVersion = currentDesktopConsistencyVersion !== window.desktopConsistencyVersion;
    } else {
    ...
    if(desktopNewVersion || mobileNewVersion) {
        ...
        if(desktopNewVersion) {
            perfDesktop = window.__LIGHTHOUSE_DESKTOP_JSON__ && {...window.__LIGHTHOUSE_DESKTOP_JSON__} || {};
            console.log("PAGESPEED SNAP - SNIFFER - sent new DESKTOP version: ", window.desktopConsistencyVersion , ' --> ', currentDesktopConsistencyVersion);
            window.desktopConsistencyVersion = currentDesktopConsistencyVersion;
        }

        if(mobileNewVersion) {
            perfMobile = window.__LIGHTHOUSE_MOBILE_JSON__ && {...window.__LIGHTHOUSE_MOBILE_JSON__} || {};
            console.log("PAGESPEED SNAP - SNIFFER - sent new MOBILE version: ", window.mobileConsistencyVersion , ' --> ', currentMobileConsistencyVersion);
...
if(window.__LIGHTHOUSE_MOBILE_JSON__) window.__LIGHTHOUSE_MOBILE_JSON__temp = {...window.__LIGHTHOUSE_MOBILE_JSON__};
if(window.__LIGHTHOUSE_DESKTOP_JSON__) window.__LIGHTHOUSE_DESKTOP_JSON__temp = {...window.__LIGHTHOUSE_DESKTOP_JSON__};
...
if(window.__LIGHTHOUSE_MOBILE_JSON__temp) window.__LIGHTHOUSE_MOBILE_JSON__ = {...window.__LIGHTHOUSE_MOBILE_JSON__temp};
if(window.__LIGHTHOUSE_DESKTOP_JSON__temp) window.__LIGHTHOUSE_DESKTOP_JSON__ = {...window.__LIGHTHOUSE_DESKTOP_JSON__temp};