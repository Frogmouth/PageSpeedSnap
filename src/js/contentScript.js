...
try {
	document.addEventListener('pss-sniffs', function (event) {
		...
		if(event.detail && (event.detail.mobile || event.detail.desktop)) {
			...
			const sniffstate = event.detail.mobile && event.detail.desktop ? 'completed' : 'partial';
			sniffstatus = {
				status : 'sending',
			...
			chrome.runtime.sendMessage(
				{
					type: 'SNIFF',
					id: getTestId(),
					state: event.detail.mobile && event.detail.desktop ? 'completed' : 'partial',
					payload: {
						id: getTestId(),
						state: event.detail.mobile && event.detail.desktop ? 'completed' : 'partial',
...
if(window.location.href.indexOf('hl=en') < 0) {
	...
	injectSniffer();
	if(sniffAndClose) {
		console.log("PAGESPEED SNAP - SNIFFER - SET PAGE REALOAD AFTER 3 MINUTES...");
